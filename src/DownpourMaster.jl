...
function Master(name::AbstractString,
  ...
  exception = :none
  master = Master(name, status, samplerstate, itersync, workers, snapper, examiner, examfac, exception, Condition(), Dict{AbstractString,Int}())
...
function synchronize(master::Master,
  ...
  SGMCMC.sample!(master.samplerstate, x -> delta)
  ...
  request_exam(master.examfac,
              ...
              params = getState(master.samplerstate))

  return (getState(master.samplerstate), master.status)
end
