...
println("Loading libraries from read_distbayes_output.jl...")
start_time = time()
include("read_distbayes_output.jl")
using MLUtilities
...
plot_avgparams = plot(
				...
				Guide.title("MNIST deep dense, 8 workers"),
				colorkey,
...
draw(PNG(string(file_suffix, "_avgparams.png"), 20cm, 15cm), plot_avgparams)
draw(PNG(string(file_suffix, "_avgparams_zoomed.png"), 20cm, 15cm), plot_avgparams_zoomed)
println("$((time() - start_time)/60) minutes\n")